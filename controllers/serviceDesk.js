...
exports.getIndexPage = async (req, res) => {
  try {
    ...
    let tasks = await Task.find({
      $and: [{ problemSolverId: req.user._id }, { isSolved: false }],
    }).lean();
    ...
    tasks.forEach((task) => {
      if (task.status === "Задача решена") {
        ...
        pastedTime > 172800000
          ? (task.status = "Задача завершена более 2 суток тому назад")
          : null;
      }
      ...
      task.createdAt = timeWorker(task.createdAt);

      task.acceptedAt = timeWorker(task.acceptedAt);
    });
...
exports.getProfile = async (req, res) => {
  try {
    const tasks = await Task.find({
      $and: [{ problemSolverId: req.user._id.toString() }, { isSolved: false }],
    }).lean();
    tasks.forEach((task) => {
      task.createdAt = timeWorker(task.createdAt);
    });
...
exports.postProblemDescription = async (req, res) => {
  try {
    ...
    task.problemSolverName = await (worker.surname +
      " " +
      worker.name +
      " " +
      worker.patronymic);
